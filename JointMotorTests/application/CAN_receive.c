...
#define REVERSE_JOINT_1_MOTOR_DIRECTION 0
#define REVERSE_JOINT_2_MOTOR_DIRECTION 0
#define REVERSE_JOINT_3_MOTOR_DIRECTION 0
#define REVERSE_JOINT_4_MOTOR_DIRECTION 0
#define REVERSE_JOINT_5_MOTOR_DIRECTION 0
#define REVERSE_JOINT_6_MOTOR_DIRECTION 0
...
uint8_t arm_joints_set_position(float joint_target_pos_ptr[7]);
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
	...
	{
		case CAN_GIMBAL_CONTROLLER_INDIVIDUAL_MOTOR_1_RX_ID:
		{
			decode_chassis_controller_rx_1(rx_data);
			break;
		}
		case CAN_GIMBAL_CONTROLLER_INDIVIDUAL_MOTOR_2_RX_ID:
		{
...
{
	engineer_robot.joint_target_pos[0] = (int16_t)((data[1] << 8) | data[0]) / RAD_TO_INT16_SCALE;
	engineer_robot.joint_target_pos[1] = (int16_t)((data[3] << 8) | data[2]) / RAD_TO_INT16_SCALE;
	engineer_robot.joint_target_pos[2] = (int16_t)((data[5] << 8) | data[4]) / RAD_TO_INT16_SCALE;
	engineer_robot.joint_target_pos[3] = (int16_t)((data[7] << 8) | data[6]) / RAD_TO_INT16_SCALE;
}

void decode_chassis_controller_rx_2(uint8_t *data)
{
	engineer_robot.joint_target_pos[4] = (int16_t)((data[1] << 8) | data[0]) / RAD_TO_INT16_SCALE;
	engineer_robot.joint_target_pos[5] = (int16_t)((data[3] << 8) | data[2]) / RAD_TO_INT16_SCALE;
	engineer_robot.joint_target_pos[6] = (int16_t)((data[5] << 8) | data[4]) / RAD_TO_INT16_SCALE;
}
...
uint8_t arm_joints_set_position(float joint_target_pos_ptr[7])
{
	...
	// encode_motor_control(CAN_JOINT_MOTOR_2_4310_ID, joint_target_pos_ptr[3], 0, 1, 1.5f, 0, 0, DM_4310, &hcan2);
	...
	encode_motor_control(CAN_JOINT_MOTOR_5_4310_ID, 0, 0, 5.0f, 0.5f, 0, 0, DM_4310, &hcan2);
	// encode_motor_control(CAN_JOINT_MOTOR_5_4310_ID, joint_target_pos_ptr[5], 0, 2, 0.5f, 0, 0, DM_4310, &hcan2);
...
// 	can_tx_message.StdId = CAN_DRIVE_MOTOR_SINGLECMD_TX_ID + 1;
// 	can_tx_message.IDE = CAN_ID_STD;
// 	can_tx_message.RTR = CAN_RTR_DATA;
// 	can_tx_message.DLC = 0x08;
// 	HAL_CAN_AddTxMessage(&hcan1, &can_tx_message, can_9015_multiangle_data, &send_mail_box);

// 	HAL_Delay(2);

// 	can_tx_message.StdId = CAN_DRIVE_MOTOR_SINGLECMD_TX_ID + 2;
// 	HAL_CAN_AddTxMessage(&hcan1, &can_tx_message, can_9015_multiangle_data, &send_mail_box);
// }
...
//	can_tx_message.StdId = CAN_JOINT_MOTOR_1_6012_ID;
...
//	can_tx_message.StdId = CAN_JOINT_MOTOR_2_4010_ID;
//	can_tx_message.ExtId = 0x00;
//	can_tx_message.RTR = CAN_RTR_DATA;
//	can_tx_message.IDE = CAN_ID_STD;
//	can_tx_message.DLC = 8;
...
//{
//	uint32_t send_mail_box;
//	can_tx_message.StdId = CAN_JOINT_MOTOR_6_6020_RX_ID;
//	can_tx_message.IDE = CAN_ID_STD;
//	can_tx_message.RTR = CAN_RTR_DATA;
//	can_tx_message.DLC = 0x08;
...
void enable_all_motor_control(uint8_t _enable)
{
	// for (uint8_t trial = 0; trial <= 3; trial++)
	{
		...
		// 	request_9015_multiangle_data();
		// }
		enable_DaMiao_motor(CAN_JOINT_MOTOR_0_4310_ID, _enable);
		enable_DaMiao_motor(CAN_JOINT_MOTOR_3_4310_ID, _enable);
		enable_DaMiao_motor(CAN_JOINT_MOTOR_4_4310_ID, _enable);
		enable_DaMiao_motor(CAN_JOINT_MOTOR_5_4310_ID, _enable);

		// for (uint8_t motor_id = CAN_HIP1_TX_ID; motor_id <= CAN_HIP4_TX_ID; motor_id++)
		// {
		// 	enable_DaMiao_motor(motor_id, _enable);
...
HAL_StatusTypeDef encode_single_6012_motor_position_control(uint16_t maxSpeed_rad, int32_t angle_rad)
{
	...
	can_send_data[1] = 0x00;
	can_send_data[2] = *((uint8_t *)&maxSpeed_deg + 0);
	can_send_data[3] = *((uint8_t *)&maxSpeed_deg + 1);
