...
echo(
echo The project for configuration "%PROJECT_TARGET%" is dowloading to FLASH. Please wait...
...
for %%I in ("%PROJECT_FOLDER%\%KEIL_PROJECT_NAME_UVPROJX%") do (
  echo UV4.exe -f -j0 %%I -t "%PROJECT_TARGET%" -o "%KEIL_PROJECT_OUTPUT_DIR%%BUILD_LOG%"
  echo(
  UV4.exe -f -j0 %%I -t "%PROJECT_TARGET%" -o "%KEIL_PROJECT_OUTPUT_DIR%%BUILD_LOG%"
  %ifErr% IF EXIST "%PROJECT_FOLDER%\%KEIL_PROJECT_OUTPUT_DIR%%BUILD_LOG%" type "%PROJECT_FOLDER%\%KEIL_PROJECT_OUTPUT_DIR%%BUILD_LOG%" &(%errExit%)
