...
echo(
echo The project for configuration "%PROJECT_TARGET%" is building. Please wait...
...
for %%I in ("%PROJECT_FOLDER%\%KEIL_PROJECT_NAME_UVPROJX%") do (
  echo UV4.exe -b -j8 %%I -t "%PROJECT_TARGET%" -o "%KEIL_PROJECT_OUTPUT_DIR%%BUILD_LOG%"
  echo(
  UV4.exe -b -j8 %%I -t "%PROJECT_TARGET%" -o "%KEIL_PROJECT_OUTPUT_DIR%%BUILD_LOG%"
